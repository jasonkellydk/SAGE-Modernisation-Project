...
PAUSE
```
It will run the game in the background and check that each replay is compatible. Use the optimized GeneralsMD
Clang/LLVM Windows x64 build with `RTS_BUILD_OPTION_DEBUG=OFF` for compatibility checks.
