																										...
																										WindowLayoutInfo *info )
{
  ...
  // read into memory
	...
	// Run the modern module parser as a compatibility check for production
	// window scripts. The legacy parser below still owns creation of
	// GameWindow instances and callbacks, and some shipped WND files contain
	// legacy extensions that the value-oriented document model does not need to
	// understand yet. Those files must continue through the legacy parser.
	...
	std::string wndSource(static_cast<std::size_t>(wndSize), '\0');
	if (inFile->seek(0, File::START) < 0
	...
	Engine::UI::WND::WNDDocument modernDocument;
	try
	{
