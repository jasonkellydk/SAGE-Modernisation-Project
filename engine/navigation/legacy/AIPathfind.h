...
public:
    explicit PathfindCellList(Bool open = false) : m_open(open) {}
    void reset();
    PathfindCell* getHead() const;
    PathfindCell* next(PathfindCell* cell) const;
    Bool empty() const;
private:
    Bool m_open;
};
...
public:
    ...
    UnsignedInt getCostSoFar() const;
    UnsignedInt getTotalCost() const;
    ...
    PathfindCell* getParentCell() const;
...
public:
	...
	void commitMacroEdge(PathfindCell *parentCell, PathfindCell *goalCell,
	...
	// The live-ground JPS adapter is enabled by default.  The fixture-only
	// switch and counters keep its comparison surface out of gameplay code.
	void setJpsAdapterEnabled(Bool enabled);
	void setDirectGroundPathEnabled(Bool enabled);
	struct JpsAdapterStats
	{
	...
	JpsAdapterStats getJpsAdapterStats() const;

	struct JpsGroundQuery
	{
...
protected:
	...
	Int examineGoalRay(PathfindCell *parentCell, PathfindCell *goalCell,
																				LocomotorSurfaceTypeMask acceptableSurfaces, Bool isCrusher,
																				Bool isHuman, Bool centerInCell, Int radius, const Object *obj,
																				ObjectID ignoredObstacleID, Bool useSpecializedIterator = true,
																				Bool allowMacroEdge = false);
	...
	*/
	...
	// Goal rays use the same legacy Bresenham phase sequence as the general
	// line iterator, but keep their hot callback path separate from line-of-
	// sight and path-tightening callers.
	Int iterateGoalRay(const ICoord2D &start, const ICoord2D &end,
	...
	void markJpsStaticTopologyDirty();
	void markJpsStaticTopologyDirty(const IRegion2D& region);
	Path *tryJpsGroundPath(const JpsGroundQuery& query, const Coord3D *from,
		const Coord3D *to);
...
private:
	...
	navigation::GoalRayQueryMemo* m_goalRayMemo;
	Bool						m_goalRayMemoEnabled;
	Bool						m_macroRayEnabled;
