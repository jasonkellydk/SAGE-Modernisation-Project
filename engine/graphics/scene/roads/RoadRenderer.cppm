...
export bool Draw_Road(SurfaceRenderer &renderer, CommandList &commands,
    ...
    std::span<const RHITextureHandle> textures, bool linear_filter)
{
    SurfaceStyle style;
    style.linear_filter = linear_filter;
    const bool masked_lightmap = Get_Environment_Lighting().parameters.pbr_options[0] < .5f
        && parameters.textured > 0.5f
