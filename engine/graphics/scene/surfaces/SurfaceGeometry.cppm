...
export module Graphics.Scene.Surfaces.Geometry;

namespace Graphics
{
export struct SurfaceVertex final
{
    std::array<float, 3> position{};
    std::array<float, 4> color{1, 1, 1, 1};
    std::array<float, 2> uv{};
};
static_assert(sizeof(SurfaceVertex) == 36);
...
export class SurfaceGeometry final
{
public:
    bool Assign(std::span<const SurfaceVertex> vertices, std::span<const std::uint32_t> indices)
    {
        constexpr auto maximum_bytes = std::numeric_limits<std::uint32_t>::max();
        if (vertices.size() > maximum_bytes / sizeof(SurfaceVertex)
    ...
    std::span<const SurfaceVertex> Vertices() const noexcept { return m_vertices; }
