...
bool ShaderClass::Load_W3D_Record(std::span<const std::byte> bytes)
{
    Assets::W3D::W3DShaderSettings record;
    if (!Assets::W3D::W3DRead_Shader(bytes, record)) return false;
    Set_Depth_Compare(static_cast<DepthCompareType>(record.depth_compare));
    Set_Depth_Mask(static_cast<DepthMaskType>(record.depth_mask));
    Set_Color_Mask(COLOR_WRITE_ENABLE);
    Set_Dst_Blend_Func(static_cast<DstBlendFuncType>(record.destination_blend));
    Set_Fog_Func(FOG_DISABLE);
    Set_Primary_Gradient(static_cast<PriGradientType>(record.primary_gradient));
    Set_Secondary_Gradient(static_cast<SecGradientType>(record.secondary_gradient));
    Set_Src_Blend_Func(static_cast<SrcBlendFuncType>(record.source_blend));
    Set_Texturing(static_cast<TexturingType>(record.texturing));
    Set_Alpha_Test(static_cast<AlphaTestType>(record.alpha_test));
    // Authored detail functions feed the draw pass. Its context selects fog
    // and color writes after loading; unrelated state such as culling survives.
    Set_Post_Detail_Color_Func(static_cast<DetailColorFuncType>(record.detail_color));
    Set_Post_Detail_Alpha_Func(static_cast<DetailAlphaFuncType>(record.detail_alpha));
    return true;
}
