	...
	CurrentGroupID(0)
{
	...
	case W3D_EMITTER_RENDER_MODE_QUAD_PARTICLES:
        Sprite = new ParticleSpriteState;
        Sprite->texture.Assign_Add_Ref(tex);
        Sprite->shader = shader;
        Sprite->frame_mode = frame_mode;
        break;
	...
	case W3D_EMITTER_RENDER_MODE_LINEGRP_TETRA:
		{
			LineGroup = new ParticleLineState;
			LineGroup->texture.Assign_Add_Ref(tex);
			LineGroup->shader = shader;
	...
	DefaultTailDiffuse(src.DefaultTailDiffuse)
{
	...
	*/
	...
	case W3D_EMITTER_RENDER_MODE_QUAD_PARTICLES:
        WWASSERT(src.Sprite);
        Sprite = new ParticleSpriteState;
        Sprite->texture = src.Sprite->texture;
        Sprite->shader = src.Sprite->shader;
        Sprite->frame_mode = src.Sprite->frame_mode;
        break;
	case W3D_EMITTER_RENDER_MODE_LINE:
		{
	...
	case W3D_EMITTER_RENDER_MODE_LINEGRP_TETRA:
		{
			...
			LineGroup = new ParticleLineState;
			LineGroup->texture = src.LineGroup->texture;
			LineGroup->shader = src.LineGroup->shader;
			TailPosition = NEW_REF( ShareBufferClass<Vector3> , (MaxNum, "ParticleBufferClass::TailPosition") );
			// TODO: Change TailPosition to Kinematic state and add
			// tail positions to bounding box
			Set_Force_Visible(1);
...
void ParticleBufferClass::Render_Particles(RenderInfoClass & rinfo)
{
    ...
    if (!Sprite || Sprite->frame_mode > 4 || active_count == 0) return;
    const unsigned columns = 1u << Sprite->frame_mode;
    const int pingpong = PingPongPosition ? WW3D::Get_Frame_Count() & 1 : 0;
    Combine_Color_And_Alpha();
    Update_Bounding_Box();
    const Vector4 default_color(ColorKeyFrameValues[0].X, ColorKeyFrameValues[0].Y,
        ColorKeyFrameValues[0].Z, AlphaKeyFrameValues[0]);
    const auto shape = RenderMode == W3D_EMITTER_RENDER_MODE_TRI_PARTICLES
        ? Graphics::SpriteShape::Triangle : Graphics::SpriteShape::Quad;
    const bool built = Sprite->geometry.Build(active_count, shape,
        Graphics::Get_Camera_Matrices().view.values, [&](std::size_t active) {
            const unsigned index = apt ? apt->Get_Array()[active] : static_cast<unsigned>(active);
            const auto& position = Position[pingpong]->Get_Array()[index];
            const auto& color = Diffuse ? Diffuse->Get_Array()[index] : default_color;
            Graphics::SpritePoint point;
            point.position = {position.X, position.Y, position.Z};
            point.color = {color.X, color.Y, color.Z, color.W};
            point.size = Size ? Size->Get_Array()[index] : SizeKeyFrameValues[0];
            point.angle = Orientation ? Orientation->Get_Array()[index] * (6.283185307179586f / 256.0f) : 0.0f;
            const unsigned frame = Frame ? Frame->Get_Array()[index] : static_cast<int>(FrameKeyFrameValues[0]) & 255;
            Graphics::Get_Sprite_Atlas_Region(frame, columns, columns, point.texture_region);
            return point;
        });
    if (!built) return;
    auto shader = Sprite->shader;
    auto* texture = Sprite->texture.Peek();
    ...
    Draw_Graphics_Material_Geometry(Sprite->geometry.Vertices(), Sprite->geometry.Indices(),
        projection, shader, {texture, nullptr}, {}, sorted ? &view_space : nullptr);
}
...
void ParticleBufferClass::Render_Line_Group(RenderInfoClass & rinfo)
{
    ...
    Update_Bounding_Box();
    if (!LineGroup || active_point_count == 0) return;
    Matrix3D offset_transform = rinfo.Camera.Get_Transform();
    offset_transform.Set_Translation(Vector3(0,0,0));
    offset_transform.Get_Orthogonal_Inverse(offset_transform);
    std::array<float,9> rotation;
    for (unsigned row=0;row<3;++row) for(unsigned column=0;column<3;++column)
        rotation[row*3+column]=offset_transform[row][column];
    const Vector4 default_color(ColorKeyFrameValues[0].X,ColorKeyFrameValues[0].Y,
        ColorKeyFrameValues[0].Z,AlphaKeyFrameValues[0]);
    const auto shape=RenderMode==W3D_EMITTER_RENDER_MODE_LINEGRP_TETRA
        ? Graphics::LineGroupShape::Tetrahedron : Graphics::LineGroupShape::Prism;
    if (!LineGroup->geometry.Build(active_point_count,shape,rotation,[&](std::size_t active) {
        const unsigned index=apt?apt->Get_Array()[active]:static_cast<unsigned>(active);
        const auto& head=Position[pingpong]->Get_Array()[index];
        const auto& tail=TailPosition->Get_Array()[index];
        const auto& color=Diffuse?Diffuse->Get_Array()[index]:default_color;
        const auto& tail_color=TailDiffuse?TailDiffuse->Get_Array()[index]:DefaultTailDiffuse;
        Graphics::LineGroupPoint point;
        point.head={head.X,head.Y,head.Z};point.tail={tail.X,tail.Y,tail.Z};
        point.head_color={color.X,color.Y,color.Z,color.W};
        point.tail_color={tail_color.X,tail_color.Y,tail_color.Z,tail_color.W};
        point.size=Size?Size->Get_Array()[index]:SizeKeyFrameValues[0];
        point.u=UCoord?UCoord->Get_Array()[index]:FrameKeyFrameValues[0];
        return point;
    })) return;
    auto& shader=LineGroup->shader;
    ...
    Draw_Graphics_Material_Geometry(LineGroup->geometry.Vertices(),LineGroup->geometry.Indices(),
        projection*view,shader,{texture,nullptr},{},sorted?&view:nullptr);
}
...
{
	if (Sprite) { auto* texture = Sprite->texture.Peek(); if (texture) texture->Add_Ref(); return texture; }
	else if (LineGroup) { auto* texture = LineGroup->texture.Peek(); if (texture) texture->Add_Ref(); return texture; }
...
{
	if (Sprite) Sprite->texture.Assign_Add_Ref(tex);
	else if (LineGroup) LineGroup->texture.Assign_Add_Ref(tex);
