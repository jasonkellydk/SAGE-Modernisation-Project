	...
	if( contain )
  {
	  ...
	  while ( iter != list.end() )
	  {
		  ...
		  if (temp)
		  {
			  ...
			  {
				  engine::debug::invariant(false, "debug failure", __FILE__, __LINE__,  "OverlordContain::createPayload: %s is full, or not valid for the payload %s!", object->getName().str(), self->m_initialPayload.name.str() );
			  }
