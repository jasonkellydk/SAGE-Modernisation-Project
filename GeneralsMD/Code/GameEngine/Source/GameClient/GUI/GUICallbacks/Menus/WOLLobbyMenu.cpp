...
#define PERF_TEST
static engine::debug::FileLog s_perfLog("Perf.txt");
...
static void populateGroupRoomListbox(GameWindow *lb)
{
	...
	for (iter = TheGameSpyInfo->getGroupRoomList()->begin(); iter != TheGameSpyInfo->getGroupRoomList()->end(); ++iter)
	{
		...
		{
			engine::debug::log_info("populateGroupRoomListbox(): groupID %d", room.m_groupID);
		...
		{
			engine::debug::log_info("populateGroupRoomListbox(): skipping QM groupID %d", room.m_groupID);
		}
	}
...
const Image* LookupSmallRankImage(Int side, Int rankPoints)
{
	...
	if (!(img)) engine::debug::log_error("*** Could not load small rank image '%s' from TheMappedImageCollection!", fullImageName.str());
	return img;
}
...
void PopulateLobbyPlayerListbox()
{
	...
	if (listboxLobbyPlayers)
	{
		...
		for (; i<maxSelectedItems; ++i)
		{
			...
			engine::debug::log_info("Saving off old selection %d (%s)", selectedIndices[i], selectedName.str());
		}
		...
		for (it = players->begin(); it != players->end(); ++it)
		{
			...
			if (info.m_flags & PEER_FLAG_OP || TheGameSpyConfig->isPlayerVIP(info.m_profileID))
			{
				...
				{
					engine::debug::log_info("Marking index %d (%s) to re-select", index, info.m_name.str());
		...
		if (!indicesToSelect.empty())
		{
			...
			while (index < count)
			{
				...
				engine::debug::log_info("Queueing up index %d to re-select", *indexIt);
				++index;
				++indexIt;
			}
...
void WOLLobbyMenuInit( WindowLayout *layout, void *userData )
{
	...
	if (!TheGameSpyInfo->getCurrentGroupRoom())
	{
		if (groupRoomToJoin)
		{
			engine::debug::log_info("WOLLobbyMenuInit() - rejoining group room %d", groupRoomToJoin);
			TheGameSpyInfo->joinGroupRoom(groupRoomToJoin);
			groupRoomToJoin = 0;
		}
		else
		{
			engine::debug::log_info("WOLLobbyMenuInit() - joining best group room");
			TheGameSpyInfo->joinBestGroupRoom();
		}
	}
	else
	{
		engine::debug::log_info("WOLLobbyMenuInit() - not joining group room because we're already in one");
	}
...
void refreshGameList( Bool forceRefresh )
{
	...
	if (forceRefresh || ((gameListRefreshTime == 0) || ((gameListRefreshTime + refreshInterval) <= timeGetTime())))
	{
		...
		{
			//engine::debug::log_info("################### refreshing game list");
			//engine::debug::log_info("gameRefreshTime=%d, refreshInterval=%d, now=%d", gameListRefreshTime, refreshInterval, timeGetTime());
		...
		//engine::debug::log_info("gameListRefreshTime: %d refreshInterval: %d", gameListRefreshTime, refreshInterval);
	}
}
//-------------------------------------------------------------------------------------------------
/** refreshPlayerList
	...
	if (TheShell->isAnimFinished() && TheTransitionHandler->isFinished() && !buttonPushed && TheGameSpyPeerMessageQueue)
	{
		...
		while (allowedMessages-- && !sawImportantMessage && TheGameSpyPeerMessageQueue->getResponse( resp ))
		{
			...
			case PeerResponse::PEERRESPONSE_JOINGROUPROOM:
				...
				{
					engine::debug::log_info("WOLLobbyMenuUpdate() - joining best group room");
					TheGameSpyInfo->joinBestGroupRoom();
				}
				populateGroupRoomListbox(comboLobbyGroupRooms);
			...
			case PeerResponse::PEERRESPONSE_ROOMUTM:
				{
					engine::debug::log_info("Putting off a UTM in the lobby");
			...
			case PeerResponse::PEERRESPONSE_JOINSTAGINGROOM:
				{
					...
					if (resp.joinStagingRoom.ok == PEERTrue)
					{
						...
						else
						{
							...
							for (Int i=0; i<MAX_SLOTS; ++i)
							{
								...
								{
									engine::debug::log_info("Saw host %s == %s in slot %d", hostName.str(), firstPlayer, i);
									isHostPresent = TRUE;
								}
							}
						}
					...
					else
					{
						...
						if (groupRoomToJoin)
						{
							engine::debug::log_info("WOLLobbyMenuUpdate() - rejoining group room %d", groupRoomToJoin);
							TheGameSpyInfo->joinGroupRoom(groupRoomToJoin);
							groupRoomToJoin = 0;
						}
						else
			...
			case PeerResponse::PEERRESPONSE_STAGINGROOM:
              ...
              room.setUseStats(resp.stagingRoom.useStats);
							...
							engine::debug::invariant((numPlayers), "numPlayers", __FILE__, __LINE__, "Game had no players!");
							//engine::debug::log_info("Saw room: hasPass=%d, allowsObservers=%d", room.getHasPassword(), room.getAllowObservers());
					...
					if (selected >= 0)
					{
						...
						if (selectedID > 0)
						{
							...
							if (srmIt != srm->end())
							{
								...
								{
									// bad crc.  don't go.
									engine::debug::log_info("WOLLobbyMenuSystem - CRC mismatch with the game I'm trying to join. My CRC's - EXE:0x%08X INI:0x%08X  Their CRC's - EXE:0x%08x INI:0x%08x", TheGlobalData->m_exeCRC, TheGlobalData->m_iniCRC, roomToJoin->getExeCRC(), roomToJoin->getIniCRC());
#if defined(RTS_DEBUG)
		...
		case GCM_SELECTED:
			{
				...
				if( controlID == comboLobbyGroupRoomsID )
				{
					...
					engine::debug::log_info("Row selected = %d", rowSelected);
					if (rowSelected >= 0)
					{
						Int groupID;
						groupID = (Int)GadgetComboBoxGetItemData(comboLobbyGroupRooms, rowSelected);
						engine::debug::log_info("ItemData was %d, current Group Room is %d", groupID, TheGameSpyInfo->getCurrentGroupRoom());
